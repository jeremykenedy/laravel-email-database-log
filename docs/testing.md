...
The suite sends actual mail through Laravel's in-memory array transport. It checks address formatting, multiple recipients, named Cc/Bcc, Unicode subjects, custom headers, plain text, MIME attachments, serialized database queue processing, missing subjects, database failure propagation, and migration rollback.
...
Chromium checks each framework in light and dark mode, searching and paging through mail, reading escaped message source, theme persistence, blocked browser storage, operating system theme changes, mobile overflow, keyboard navigation, and WCAG A/AA rules through axe. Framework selection stays fixed throughout navigation using a test-only request header. Test application keys are generated at runtime. Tests use no production mail or database credentials.
