...
@section('content')
    ...
    <div class="el-heading"><div><p class="el-eyebrow">MESSAGE #{{ $log->id }}</p><h1>{{ $log->subject !== '' ? $log->subject : '(No subject)' }}</h1><p class="el-muted">Recorded {{ $log->date->format('Y-m-d H:i:s') }}</p></div><span class="el-badge">Read only</span></div>
