...
@section('content')
    ...
    @if(config('laravel-email-database-log-ui.ui_kit') && class_exists('Jeremykenedy\LaravelUiKit\Providers\UiKitServiceProvider'))
        @include('email-log::partials.ui-kit-notice')
    @endif
        ...
        @else
            <div class="el-table-scroll" role="region" aria-label="Recorded emails" tabindex="0">
                <table class="el-table {{ $classes['table'] }}">
                    ...
                    <tbody>
                    @foreach($logs as $log)
                        <tr><td><a class="el-subject" href="{{ route('email-log.show', $log->id) }}">{{ $log->subject !== '' ? $log->subject : '(No subject)' }}</a><span class="el-sender">{{ $log->from ?: 'No sender' }}</span></td><td>{{ $log->to ?: 'No direct recipient' }}</td><td class="el-date"><time>{{ $log->date }}</time></td><td><a class="el-link" href="{{ route('email-log.show', $log->id) }}" aria-label="View email {{ $log->id }}">View</a></td></tr>
                    @endforeach
                    </tbody>
                </table>
            </div>
