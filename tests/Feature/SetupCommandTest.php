...
class SetupCommandTest extends TestCase
{
    ...
    public function test_switch_requires_a_framework_and_preserves_customizations(): void
    {
        $this->assertSame(1, $this->runCommand('email-log:switch'));
        ...
        $config = file_get_contents(config_path('laravel-email-database-log.php'));
        $this->assertSame(0, $this->runCommand('email-log:switch', ['--framework' => 'tailwind']));
        $this->assertSame('tailwind', $this->settings()['framework']);
        $this->assertSame('Custom view', file_get_contents($path));
        $this->assertSame($config, file_get_contents(config_path('laravel-email-database-log.php')));
    }
