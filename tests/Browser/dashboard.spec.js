...
for (const framework of ["standalone", "bootstrap5", "tailwind"]) {
  for (const theme of ["light", "dark"]) {
    test(`${framework} ${theme}: browse, search, read safely, and keep theme`, async ({
      ...
      await context.setExtraHTTPHeaders({ "X-Email-Log-Framework": framework });
      const assertStylesheet = async () => {
        await expect(
          page.locator(`link[href="/${framework}.css"]`),
        ).toHaveCount(framework === "standalone" ? 0 : 1);
      };
  ...
  test(`${framework}: mobile layout and keyboard access`, async ({
    page,
    context,
  }) => {
